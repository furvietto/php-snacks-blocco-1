...
<body>
    <?php   
    $array = [];
    foreach ($ads as $key => $searchTrue) {
        if ($searchTrue["is_active"]) {
            $array[] = $searchTrue;
        }
    }
    $adsrand = $array[rand(0,2)];
    if ($adsrand["is_active"] ) {
        ?>
         <img src="<?php echo $adsrand["image_path"] ?>" alt="">
         <a href="<?php echo $adsrand["link"] ?>">clicca qui</a>
         <?php
    }
    ?>
</body>
